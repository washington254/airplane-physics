...
const usePlaneControls = (planeRef, camera) => {
  ...
  useEffect(() => {
    ...
    const MAX_PITCH_ANGLE = THREE.MathUtils.degToRad(45);
    const MIN_PITCH_ANGLE = -MAX_PITCH_ANGLE;

    const MAX_ROLL_ANGLE = THREE.MathUtils.degToRad(45);
      ...
      if (planeRef.current) {
        ...
        const yawAcceleration = THREE.MathUtils.clamp(
          THREE.MathUtils.lerp(
            0.0001,
            acceleration,
            Math.abs(planeSpeed.current) / maxVelocity,
          ),
          0.0001,
        ...
        yawVelocity.current = clamp(
          yawVelocity.current,
          -maxVelocity,
          maxVelocity,
        );
        pitchVelocity.current = clamp(
          pitchVelocity.current,
          -maxVelocity,
          maxVelocity,
        );
        rollVelocity.current = clamp(
        ...
        if (
          ...
          if (
            currentEuler.x > MAX_PITCH_ANGLE ||
            currentEuler.x < MIN_PITCH_ANGLE
          ) {
            const targetPitch =
              currentEuler.x > MAX_PITCH_ANGLE
                ? MAX_PITCH_ANGLE
          ...
          if (
            currentEuler.z > MAX_ROLL_ANGLE ||
            currentEuler.z < MIN_ROLL_ANGLE
          ) {
            const targetRoll =
              currentEuler.z > MAX_ROLL_ANGLE ? MAX_ROLL_ANGLE : MIN_ROLL_ANGLE;
